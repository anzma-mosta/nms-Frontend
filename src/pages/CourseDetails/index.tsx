            ...
            <div className="lg:col-span-8 space-y-8">
              <Reveal>
                <div className="flex flex-wrap gap-3 mb-6">
                  ...
                  <Badge variant="outline" className="text-white border-white/20 px-4 py-1.5 rounded-full backdrop-blur-sm bg-white/5">
                ...
                <div className="flex flex-wrap gap-8 items-center pt-6 border-t border-white/10 mt-8">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-xs text-slate-400 uppercase tracking-widest">{t("course_details.instructor")}</p>
                      <p className="font-bold text-lg hover:text-primary transition-colors cursor-pointer">{course.instructor}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex-grow flex justify-end">
                    ...
                    >
                      {isEnrolled ? t("course_details.visitor_view") : t("course_details.student_view")}
                    </Button>
                  </div>
                </div>
                  ...
                  <div className="mt-8 flex items-center gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    ...
                    <div className="flex-1 max-w-xs">
                      <div className="flex justify-between text-sm mb-2">
                        <span className="text-slate-400">{t("lesson.progress")}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-12 pb-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { icon: Clock, label: t("course_details.duration_label"), value: course.duration },
                { icon: BookOpen, label: t("course_details.lessons_label"), value: "85+" },
                { icon: Target, label: t("course_details.level"), value: course.level },
                { icon: Award, label: t("course_details.certificate"), value: t("course_details.certified") },
              ].map((stat, i) => (
            ...
            <Reveal>
              <div>
                <div className="flex items-center justify-between mb-8">
                  <h2 className="text-3xl font-bold">{t("course_details.curriculum_title")}</h2>
                  <div className="text-sm text-muted-foreground bg-muted px-4 py-2 rounded-full">
                    <span className="font-bold text-foreground">12</span> {t("course_details.sections_label")} • <span className="font-bold text-foreground">85</span> {t("course_details.lessons_label")}
                  </div>
                </div>
                            ...
                            <div className="px-6 pb-6 space-y-2 border-t border-border/50 pt-4">
                              {section.lessons.map((lesson, lIdx) => {
                                ...
                                return hasAccess ? (
                                  ...
                                  >
                                    <div className="flex items-center gap-3">
                                      <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isEnrolled ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
                                  ...
                                  <div key={lIdx} className="flex items-center justify-between p-3 rounded-xl opacity-60 cursor-not-allowed">
                                    <div className="flex items-center gap-3">
                ...
                <div className="bg-card text-foreground rounded-[2.5rem] overflow-hidden shadow-2xl shadow-primary/10 border-4 border-white dark:border-slate-900">
                  <div className="relative aspect-video group cursor-pointer">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                      ...
                      <p className="text-white font-bold mt-4 tracking-wide">{t("course_details.preview_course")}</p>
                    </div>
                  </div>
                  ...
                  <div className="p-8">
                    {isEnrolled ? (
                      <div className="space-y-6">
                        <div className="bg-primary/5 rounded-2xl p-6 border border-primary/10">
                          <p className="text-sm text-muted-foreground mb-2">{t("course_details.already_enrolled")}</p>
                          <div className="flex items-center justify-between mb-4">
                            <span className="text-2xl font-black text-primary">{course.progress}%</span>
                            <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{t("course_details.completed_status")}</span>
                          </div>
                        ...
                        <Link to={ROUTES.LESSON.replace(":courseId", id || "1").replace(":lessonId", "l1")}>
                      ...
                      <>
                        <div className="flex items-center gap-4 mb-8">
                          ...
                          <div className="flex flex-col">
                            <span className="text-lg text-muted-foreground line-through decoration-red-500/50 decoration-2">{course.oldPrice}</span>
                            <Badge className="bg-green-500 text-white border-none w-fit text-[10px] py-0 px-2 rounded-sm">
                              {t("course_details.discount_percent", { percent: 50 })}
                            </Badge>
                          </div>
                        </div>
                    ...
                    <div className="space-y-5 mt-8">
                      ...
                      <div className="space-y-4">
                        {[
                          { icon: Zap, text: t("course_details.lifetime_access") },
                          { icon: ShieldCheck, text: t("course_details.money_back_guarantee") },
                          { icon: BarChart, text: t("course_details.practical_projects") },
      ...
      <section className="bg-muted/20 py-20 mt-20">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 opacity-50">
            {[1,2,3].map(i => (
              <div key={i} className="h-64 bg-card border-2 shadow-md rounded-3xl animate-pulse" />
            ))}
          </div>
        </div>
      </section>
